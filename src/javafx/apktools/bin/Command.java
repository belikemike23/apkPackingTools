...
//            XMLWriter writer = new XMLWriter(new FileOutputStream(androidManifestFile), format);
     ...
     * @param pac      要修改的信息
     */
    public boolean updatePackageName(String appFolderName, Pac pac){
        if(pac == null){
            return false;
        }
        try{
            ...
            root.attribute("package").setText(pac.getName());
            callback("更新 AndroidManifest.xml package='" + pac.getName());
            XMLWriter writer = new XMLWriter(new FileOutputStream(androidManifestFile));
            writer.write(document);
            writer.close();
            callback("更新 AndroidManifest.xml 完成 ~ ");
     ...
     */
    public boolean updateApkToolYmlVersion(String appFolderName, String versionName ,String versionCode) {
        try {
    ...
    * 修改电信、MM渠道号
    * */
    public boolean updateCarrierInfo(String appFolderName ,String ctChannel , String mmChannel){
        try{
            File ctChannelFile = new File(appFolderName + File.separator + "assets"+File.separator+"egame_channel.txt");
            if(ctChannelFile.exists()){
               OutputStream os = new FileOutputStream(ctChannelFile);
                try{
                    os.write(ctChannel.getBytes());
                    os.close();
                }catch (IOException e ){
                    e.printStackTrace();
                }
            ...
            if(mmChannelFile.exists()) {
                ...
                channelElement.setText(mmChannel);
                XMLWriter writer = new XMLWriter(new FileWriter(mmChannelFile));
                writer.write(document);
                writer.flush();
                writer.close();
            }
    ...
    /**
     * 修改解密之后的skynet_config文件之中的gameversion
     *
     * @param appFolderName 文件所在的文件夹路径
     * @param gameVersion   要改的游戏版本
     */
    public boolean updateSkynetconfig(String appFolderName, String skynetconfig,String ledouChannel,String gameVersion,String sdkVersion) {
        try {
            JSONObject jsonObject=new JSONObject(skynetconfig);
            jsonObject.putOpt("channel_id",ledouChannel);
            jsonObject.putOpt("game_version",gameVersion);
            jsonObject.putOpt("sdk_version",sdkVersion);
            String res=jsonObject.toString();
            res = res.replaceAll("@@", "\"").replace("+","%2B");
            StringBuffer sb = new StringBuffer();
            sb.append("[");
            for(int i = 0; i < res.length(); i += 100) {
                ...
                sb.append("\"" + Des.encode(KEY, temp).replace("\n", "").replace("\r", "") + "\"");
                if(!isLast) {
                    sb.append(',');
                } else {
                    sb.append(']');
            ...
            File skynetconfigfile = new File(appFolderName + File.separator +"assets"+File.separator+"skynet_config.txt");
            if(!skynetconfigfile.exists()){
                skynetconfigfile = new File(appFolderName + File.separator + "assets"+File.separator+"ids_config.txt");
            }
            FileOutputStream out = new FileOutputStream(skynetconfigfile);
            out.write(sb.toString().getBytes());
            close(out);
            callback("更新 skynetconfig 完成 ~ ");
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
     ...
     * @return 版本号
     * 使用新的versionCode获取方式 by Mike@20160822
